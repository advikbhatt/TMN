...
document.addEventListener('scroll', function () {
	...
	const logo = document.getElementById('colorchanger');

	if (window.scrollY > 50) {
		header.classList.add('header-scrolled');
		logo.src = 'TMNwhite.png';
	} else {
...
document.addEventListener("DOMContentLoaded", function () {
	var triangleBox = document.querySelector(".triangle_box");
	var popup = document.getElementById("popupMessage");

	// Show the popup message when triangle_box is clicked
	triangleBox.addEventListener("click", function () {
		popup.classList.toggle("show");
	});

	// Close the popup message if user clicks outside of it
	document.addEventListener("click", function (event) {
		if (!popup.contains(event.target) && event.target !== triangleBox) {
			popup.classList.remove("show");
		}
	});
});
document.addEventListener("DOMContentLoaded", function() {
	var triangleBoxes = document.querySelectorAll(".triangle_box");

	triangleBoxes.forEach(function(box) {
		box.addEventListener("click", function() {
			var popupId = box.getAttribute("data-popup-id");
